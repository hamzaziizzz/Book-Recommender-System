...
popularity_df = pickle.load(open("./model/popular.pkl", "rb"))
...
def recommend():
    ...
    # index fetch
    indices = np.where(pt.index == user_input)[0][0]
    similar_items = sorted(list(enumerate(similarity_scores[indices])), key=lambda x: x[1], reverse=True)[1:6]
